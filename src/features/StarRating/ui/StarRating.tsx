...
export const StarRating = memo(({ className, count }: StarRatingProps) => {
  const [selected, setSelected] = useState<number>(-1);
  const [hoverStar, setHoverStar] = useState<number>(-1);
  const [modalOpen, setModalOpen] = useState<boolean>(false);

  const starts = Array(count).fill(0);

  const debouncedOpenModal = useDebounce(() => {
    setModalOpen(true);
  }, 1000);
                ...
                <g>
                  <path
                    className={classNames(
                      ...
                      index <= hoverStar ? styles.hovered : '',
                    )}
                    fill='transparent'
                    stroke='#ffffff'
